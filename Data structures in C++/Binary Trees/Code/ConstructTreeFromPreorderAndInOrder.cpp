...
BinaryTreeNode<int> *buildTreeHelper(int *preorder, int preStart, int preEnd, int *inorder, int inStart, int inEnd)
{
    ...
    int preLeftEnd = preStart + rootIndex;
    int inLeftStart = inStart + 0;
    int inLeftEnd = inStart + rootIndex - 1;
    int preRightStart = preLeftEnd + 1;
    int preRightEnd = preEnd;
    int inRightStart = inLeftEnd + 2;
    int inRightEnd = inEnd;
    BinaryTreeNode<int> *buildLeftChild = buildTreeHelper(preorder, preLeftStart, preLeftEnd, inorder, inLeftStart, inLeftEnd);
    BinaryTreeNode<int> *buildRightChild = buildTreeHelper(preorder, preRightStart, preRightEnd, inorder, inRightStart, inRightEnd);
    root->left = buildLeftChild;
    root->right = buildRightChild;
    return root;
}
